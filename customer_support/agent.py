...
def check_order_status(order_id: str) -> dict:
    ...
    Returns:
        dict: Order status information.
            On success: {'status': 'success', 'order_id': '...', 'order_status': '...', 'details': {...}}
            On error: {'status': 'error', 'error_type': 'not_found/invalid_format', 'error_message': '...'}
    """
    ...
    if not order_id.startswith("ORD"):
        return {
            ...
            "error_message": f"Order ID must start with 'ORD' (e.g., ORD123)."
        }
...
def process_refund(order_id: str, reason: str) -> dict:
    ...
    if order["status"] == "delivered":
        ...
        return {
            ...
            "error_message": f"Cannot refund order in '{order['status']}. Only delivered orders can be refunded."
        }
...
def escalate_to_supervisor(issue_summary: str, order_id: str) -> dict:
    ...
    Returns:
        dict: Escalation confirmation.
            Always retuns: {'status': 'success', 'ticket_id': 'TICKET###'}
    """
